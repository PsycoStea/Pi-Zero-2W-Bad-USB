...
| `INJECT_MOD <modifier_code>` | Sets the held modifier byte, applied to every subsequent keystroke until released. Combine modifier bits: `0x01` CTRL, `0x02` SHIFT, `0x04` ALT, `0x08` GUI. Example: `0x03` = CTRL+SHIFT. Inject `0x00` to release all. |
| `LAYOUT <name>` | Select the keyboard layout used to translate characters to HID usage IDs. Built-in: `US` (default) and `UK`. Add more by dropping a Python module into `keymaps/`. |
